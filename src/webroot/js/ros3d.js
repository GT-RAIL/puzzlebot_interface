...
ROS3D.DepthCloud = function(options) {
  ...
  if(!options.pose){
    this.pose = {position:{x:0.0,y:0.0,z:0},orientation:{x:0,y:0.0,z:0.0}};
  }
  else{
  ...
  this.picking_vertex_shader = [
    ...
    '       P.g=(position.x)/(width/4.0);',
    '    }',
    '    else if (position.x< (width/2.0)){',
    '       P.g=(position.x- (width/4.0))/(width/4.0);',
    '       P.r+=0.015625;',
    '    }',
    '    else if (position.x< (3.0*width/4.0)){',
    '       P.g=(position.x- (width/2.0))/(width/4.0);',
    '       P.r+=0.03125;',
    '    }',
    '    else{',
    '       P.g=(position.x- (3.0*width/4.0))/(width/4.0);',
    '       P.r+=0.046875;',
    '    }',
    '    if ((height -position.y)< (height/4.0)){',
    '       P.b=((height -position.y)/(height/4.0));',
    '    }',
    '    else if ((height -position.y)< (height/2.0)){',
    '       P.b=((height -position.y)- (height/4.0))/(height/4.0);',
    '       P.r+=0.00390625;',
    '    }',
    '    else if ((height -position.y)< (3.0*height/4.0)){',
    '       P.b=((height -position.y)- (height/2.0))/(height/4.0);',
    '       P.r+=0.0078125;',
    '    }',
    '    else{',
    '       P.b=((height -position.y)- (3.0*height/4.0))/(height/4.0);',
    '       P.r+=0.01171875;',
    '    }',
    '    P.a= ((- z + zOffset / 1000.0)) /5.0;',
    '  }',
    '  ',
    '  gl_PointSize = pointSize;',
    '  gl_Position = projectionMatrix * modelViewMatrix * pos;',
          ...
          if (that.viewer.mouseHandler.deviceX>0 && that.viewer.mouseHandler.deviceY>0){
            ...
            if(min_z_index!==-1){
              ...
              if(pixelBuffer[3]!==0 && pixelBuffer[3]!==255){
                  var x_position=pixelBuffer[1]*(that.width/4)/255;
                  // x_position+=((pixelBuffer[0]===64 || pixelBuffer[0]===191 )?(that.width/4):0);
                  if(pixelBuffer[0]>=8){
                    x_position+=(that.width/2);
                    pixelBuffer[0]-=8;
                  }
                  if(pixelBuffer[0]>=4){
                    x_position+=(that.width/4);
                    pixelBuffer[0]-=4;
                  }
                  // x_position+=(pixelBuffer[0]>8)?(that.width/2):0;
                  var y_position=pixelBuffer[2]*(that.height/4)/255;
                  y_position+=pixelBuffer[0]*(that.height/4);
 ...
 * @param control - the control to use
 */
ROS3D.InteractiveMarker.prototype.feedbackEventWithPoint = function(type, control, point) {
  this.dispatchEvent({
    type : type,
    position : point,
    orientation : this.quaternion.clone(),
...
ROS3D.InteractiveMarker.prototype.buttonClick = function(control, event3d) {
  event3d.stopPropagation();
  this.feedbackEventWithPoint('user-button-click', control, event3d.intersection.point);
};
...
ROS3D.InteractiveMarkerHandle.prototype.onButtonClick = function(event) {
  this.sendFeedback(ROS3D.INTERACTIVE_MARKER_BUTTON_CLICK, event.position, 0,
      event.controlName);
};
...
ROS3D.Marker = function(options) {
  ...
  switch (message.type) {
    ...
    case ROS3D.MARKER_SPHERE:
      ...
      var sphereGeom = new THREE.SphereGeometry(0.5, 16, 8);
 ...
 *  * center (optional) - specifies the starting center for the orbit
 */
...
ROS3D.Viewer.prototype.changeCamera = function(cameraID) {
  if (cameraID<this.cameras.length && cameraID>=0){
    ...
    this.cameraControls.camera.position.x = position.x;
    this.cameraControls.camera.position.y = position.y;
    this.cameraControls.camera.position.z = position.z;
    this.cameraControls.center.x = this.cameras[cameraID].center.x;
    this.cameraControls.center.y = this.cameras[cameraID].center.y;
    this.cameraControls.center.z = this.cameras[cameraID].center.z;
    //this.cameraControls.camera.lookAt(this.cameraControls.center);
 ...
 * @param center (optional) - initial center position
 */
