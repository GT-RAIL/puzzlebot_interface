//mjpegcanvas 2 
//This version requires you to have RosLibJS
// MJPEGCANVAS.MultiStreamViewer=function(options,EventEmitter){
//     var that = this;
//     var viewer = new EventEmitter({options});
...
var MJPEGCANVAS = {
  ...
  Viewer : function(options){
	...
	//keep a copy of the variable
	var that = this;
	//get the ID of the DIV that we are writing to
	var divID = options.divID;
	//get the width of the mJPEGCanvas that we are going to insert
	this.width = options.width;
	//get the height of the mJPEGCanvas that we are going to insert	  
	this.height = options.height;
	//get the host of the stream of the video
	this.host = options.host;
	//get the port of the stream of the video. If not set it defaults to 8080
	this.port = options.port || 8080;
	//gets the quality of the video
	this.quality = options.quality;

	//the topics that we are subscribing to for the different streams
	var topics = options.topics;
	//the labels of the topics that we are subscribing to
	var labels = options.labels;
	//the default stream of the video
	var defaultStream = options.defaultStream || 0;
	//the current topic that we are getting the video from
	var currentTopic = topics[defaultStream];
	var markerTopic = options.markerTopicName;
	//get the object of the transform frame for a frame of reference
	...
	this.transform={'rotation':{'x':0,'y':0,'z':0},'translation':{'x':0,'y':0,'z':0}}
	...
	*/
	...
	// create no image initially
  	...
  	function draw() {
    	...
    	if (that.image.width * that.image.height > 0) {
      		...
      		context.drawImage(MJPEGCANVAS.errorImage, (that.width - (that.width / 2)) / 2,
	...
	this.changeStream = function(topic) {
	    ...
	    var src = 'http://' + that.host + ':' + that.port + '/stream?topic=' + topic;
	    console.log(src)
	    // add various options
	    src += '&width=' + that.width;
	    src += '&height=' + that.height;
	    if (that.quality > 0) {
	      src += '&quality=' + that.quality;
	    }
	    if (that.invert) {
	      src += '&invert=' + that.invert;
	    }
	    that.image.src = src;
	...
	setInterval(draw, drawInterval);
  	...
  	MultiStreamViewer : function(options,EventEmitter){ 	
  		options.topic=options.topics[0]
  		return new MJPEGCANVAS.Viewer(options)
  	},
  	errorImage: new Image(),
		interaction: function(linear,angular){
	},
  	convertWorldCoordinatesToImageCoordinates:function(transform,x,y,z,width,height){
  	...
  	convertImageCoordinatestoWorldCoordinates:function(transform,x,y,z,width,height){
		x=x + x* Math.cos(transform.rotation.z) * Math.sin(transform.rotation.y)
		y=y + y* Math.sin(transform.rotation.z)
		z=z + z* Math.cos(transform.rotation.z) * Math.cos(transform.rotation.y)
		x=x-transform.translation.x;
